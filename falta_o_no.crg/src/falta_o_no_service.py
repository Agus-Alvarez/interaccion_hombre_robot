...
class FaltaONoService(ALModule):
    ...
    def iniciar_juego(self):
        ...
        if self.imagenes:
            self.tabletService.loadApplication("falta_o_no")  # Carga la aplicación web
            self.tabletService.showWebview("http://127.0.0.1/apps/falta_o_no/html/index.html") # Muestra index.html
            self.mostrar_imagen()
    ...
    def mostrar_imagen(self):
        if 0 <= self.indice_imagen_actual < len(self.imagenes):
            ...
            #  Llamamos a la función JavaScript para cambiar la imagen
            self.tabletService.executeJS("cambiarImagen('" + ruta_imagen + "');")
    ...
    #  Esta función será llamada desde JavaScript en la página web
    def pepperRespuesta(self, respuesta):
        self.verificar_respuesta(respuesta)

    def main(session):
        global module
        module = FaltaONoService("FaltaONoService")
        #  Necesitamos crear una instancia del servicio para poder usar pepperRespuesta
        global falta_o_no_service
        falta_o_no_service = module
        session.registerService("FaltaONoService", module)
